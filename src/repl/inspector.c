...
static bool repl_preview_member_complete(
  ...
  if (prefix_len > 0 && !repl_preview_expr_is_ident(line + prefix_start, prefix_len))
    return false;
  ...
  while (js_prop_iter_next(&iter, &key, &key_len, &value)) {
    (void)value;
    if (!repl_preview_expr_is_ident(key, key_len)) continue;
    if (key_len < prefix_len || memcmp(key, line + prefix_start, prefix_len) != 0)
      continue;
  ...
  size_t suffix_bytes = completed_len - prefix_len;
  if (suffix_bytes >= suffix_len) suffix_bytes = suffix_len - 1;
  memcpy(suffix_out, match + prefix_len, suffix_bytes);
  suffix_out[suffix_bytes] = '\0';
  return suffix_out[0] != '\0';
}
...
bool repl_preview_compute(
  ...
  ant_value_t value = js_mkundef();
  if (inspector_eval_safe_member_expr(js, preview_expr, preview_expr_len, &value))
