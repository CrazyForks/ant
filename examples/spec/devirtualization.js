...
// Exercises the JIT's monomorphic method-call devirtualization. Each driver is
// run well past the JIT threshold (100 calls) so the hot caller is compiled and
// its `obj.method()` sites are speculatively inlined. Because the harness runs
// interpreted first and then JITted, any miscompilation shows up as a warmed
// result that diverges from the reference constant computed below.
...
// Alternate shapes at the same call site: the speculative guard must miss and
// route to the generic dispatch without corrupting results.
let polyAcc = 0;
for (let i = 0; i < HOT; i++) {
...
let normalOk = true;
for (let i = 0; i < HOT; i++) {
  const n = i + 1000; // stay clear of the throwing input on the hot path
